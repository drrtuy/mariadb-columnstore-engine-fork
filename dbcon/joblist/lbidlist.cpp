...
#endif
idblog("Added Mx/Mn to partitionVector");
...
idblog("Added Mx/Mn to partitionVector, min " << std::hex << mmp->min << ", max " << mmp->max);
idblog("                              , col data type" << ((int)colDataType));
                mmp->isValid = retVal;
                mmp->blksScanned = 0;
    ...
    for (uint32_t i = 0; i < lbidPartitionVector.size(); i++)
    {
        ...
        if ( (lbid >= mmp->lbid) && (lbid < mmp->lbidmax) )
        {
            ...
            if (mmp->isValid == BRM::CP_INVALID)
            {
                ...
                {
idblog("LBID " << mmp->lbid << ": updating rrange for character values, min " << std::hex << ((int64_t)mmp->min) << ", max " << ((int64_t)mmp->max) << ", new min " << ((int64_t)min) << ", new max " << ((int64_t)max));
                    ...
                    if (datatypes::TCharShort::strnncollsp(cs, max, mmp->max, type.colWidth) > 0 ||
                        ...
                        mmp->max = max;
idblog("LBID " << mmp->lbid << ": updated rrange for character values, min " << std::hex << mmp->min << ", max " << mmp->max);
                }
                else if (dictScan || datatypes::isUnsigned(type.colDataType))
                {
idblog("LBID " << mmp->lbid << ": updating rrange for unsigned values, min " << ((uint64_t)min) << ", max " << (((uint64_t)max)));
                    if (static_cast<uint64_t>(min) < static_cast<uint64_t>(mmp->min))
                        mmp->min = min;

                    if (static_cast<uint64_t>(max) > static_cast<uint64_t>(mmp->max))
                        mmp->max = max;
idblog("LBID " << mmp->lbid << ": updated rrange for unsigned values, min " << ((uint64_t)mmp->min) << ", max " << (((uint64_t)mmp->max)));
                }
                else
                {
                    if (typeid(T) == typeid(int128_t))
...
idblog("pushing cpinfo for LBID " << mmp->lbid << ", min " << std::hex << mmp->min << ", max " << mmp->max);
                                        ...
                                        const uint8_t BOP, bool isDict)
{
    ...
    int64_t value = 0;
    int128_t bigValue = 0;
    // this really is a sorta hack. We do not have means to discern between short char column and tokens
    // using only ct.colDataType and width.
    // But, command we test against, has this information in the "getIsDict()" method.
    // So we change signedness and "characterness" based on whether we work with tokens' column or not.
...
{
idblog("checking null, is char " << ((int)bIsChar) << ", width " << ct.colWidth << std::hex << ", value " << value);
...
template
void LBIDList::UpdateMinMax<int128_t>(int128_t min, int128_t max, int64_t lbid, bool dictScan,
                                      const execplan::CalpontSystemCatalog::ColType & type, bool validData = true);

template
void LBIDList::UpdateMinMax<int64_t>(int64_t min, int64_t max, int64_t lbid, bool dictScan,
                                     const execplan::CalpontSystemCatalog::ColType & type, bool validData = true);

...
CartJoinStep::CartJoinStep(const JobInfo& jobInfo) :
    JobStep(jobInfo),
    joinType(INIT),
    fTableOID1(0),
    fTableOID2(0),
...
// 	cout << "CartJoinStep::run(): fOutputJobStepAssociation.outSize = " << fOutputJobStepAssociation.outSize() << ", fDelivery = " << boolalpha << fDelivery << endl;
...
void CartJoinStep::smallRunnerFcn(uint32_t index)
{
    uint64_t i;
...
//  			smallRG.getRowCount() << endl;
    ...
    {
        ostringstream oss;
        oss << "CartJoinStep::smallRunnerFcn failed due to " << boost::diagnostic_information(e);
    ...
    {
        ostringstream oss;
        oss << "CartJoinStep::smallRunnerFcn failed due to " << e.what();
        fLogger->logMessage(logging::LOG_TYPE_ERROR, oss.str());
        status(logging::ERR_EXEMGR_MALFUNCTION);
    }
    catch (...)
    {
        fLogger->logMessage(logging::LOG_TYPE_ERROR, "CartJoinStep::smallRunnerFcn failed due to an unknown reason (...)");
        status(logging::ERR_EXEMGR_MALFUNCTION);
    }
...
void CartJoinStep::forwardCPData()
{
    uint32_t i, col;
...
void CartJoinStep::djsReaderFcn(int index)
{
    /*
...
uint32_t CartJoinStep::nextBand(messageqcpp::ByteStream& bs)
{
    RGData oneRG;
    bool more;
    uint32_t ret = 0;
...
void CartJoinStep::setLargeSideBPS(BatchPrimitive* b)
{
    largeBPS = dynamic_cast<TupleBPS*>(b);
}

void CartJoinStep::startAdjoiningSteps()
{
    if (largeBPS)
        largeBPS->run();
}
...
const string CartJoinStep::toString() const
{
    ostringstream oss;
    size_t idlsz = fInputJobStepAssociation.outSize();
    idbassert(idlsz > 1);
    oss << "CartJoinStep    ses:" << fSessionId << " st:" << fStepId;
    oss << omitOidInDL;
...
//------------------------------------------------------------------------------
void CartJoinStep::errorLogging(const string& msg, int err) const
{
...
void CartJoinStep::addSmallSideRG(const vector<rowgroup::RowGroup>& rgs,
...
void CartJoinStep::addJoinKeyIndex(const vector<JoinType>& jt,
...
void CartJoinStep::configSmallSideRG(const vector<RowGroup>& rgs, const vector<string>& tnames)
{
    smallTableNames.insert(smallTableNames.begin(), tnames.begin(), tnames.end());
    smallRGs.insert(smallRGs.begin(), rgs.begin(), rgs.end());
}

void CartJoinStep::configLargeSideRG(const RowGroup& rg)
{
    largeRG = rg;
}

void CartJoinStep::configJoinKeyIndex(const vector<JoinType>& jt,
        const vector<bool>& typeless,
        const vector<vector<uint32_t> >& smallkey,
        const vector<vector<uint32_t> >& largekey)
{
...
void CartJoinStep::setOutputRowGroup(const RowGroup& rg)
{
    outputRG = rg;
}

execplan::CalpontSystemCatalog::OID CartJoinStep::smallSideKeyOID(uint32_t s, uint32_t k) const
{
    return smallRGs[s].getOIDs()[smallSideKeys[s][k]];
}

execplan::CalpontSystemCatalog::OID CartJoinStep::largeSideKeyOID(uint32_t s, uint32_t k) const
{
    return largeRG.getOIDs()[largeSideKeys[s][k]];
}

void CartJoinStep::addFcnExpGroup2(const boost::shared_ptr<execplan::ParseTree>& fe)
{
    if (!fe2)
...
void CartJoinStep::setFcnExpGroup3(const vector<boost::shared_ptr<execplan::ReturnedColumn> >& v)
{
    if (!fe2)
        fe2.reset(new funcexp::FuncExpWrapper());
...
void CartJoinStep::setFE23Output(const rowgroup::RowGroup& rg)
{
    fe2Output = rg;
}

const rowgroup::RowGroup& CartJoinStep::getDeliveredRowGroup() const
{
    if (fe2)
        return fe2Output;
...
void CartJoinStep::deliverStringTableRowGroup(bool b)
{
    if (fe2)
...
bool CartJoinStep::deliverStringTableRowGroup() const
{
    if (fe2)
...
void CartJoinStep::addJoinFilter(boost::shared_ptr<execplan::ParseTree> pt, uint32_t index)
{
...
bool CartJoinStep::hasJoinFilter(uint32_t index) const
{
...
boost::shared_ptr<funcexp::FuncExpWrapper> CartJoinStep::getJoinFilter(uint32_t index) const
{
    for (uint32_t i = 0; i < feIndexes.size(); i++)
        if (feIndexes[i] == static_cast<int>(index))
            return fe[i];
...
void CartJoinStep::setJoinFilterInputRG(const rowgroup::RowGroup& rg)
{
    joinFilterRG = rg;
}

void CartJoinStep::startJoinThreads()
{
    uint32_t i;
...
void CartJoinStep::finishSmallOuterJoin()
{
...
void CartJoinStep::joinRunnerFcn(uint32_t threadID)
{
...
void CartJoinStep::makeDupList(const RowGroup& rg)
{
...
void CartJoinStep::processDupList(uint32_t threadID, RowGroup& rg,
...
void CartJoinStep::processFE2(RowGroup& input, RowGroup& output, Row& inRow, Row& outRow,
...
void CartJoinStep::sendResult(const vector<RGData>& res)
{
...
void CartJoinStep::grabSomeWork(vector<RGData>* work)
{
...
void CartJoinStep::joinOneRG(uint32_t threadID, vector<RGData>* out,
...
void CartJoinStep::generateJoinResultSet(const vector<vector<Row::Pointer> >& joinerOutput,
...
void CartJoinStep::segregateJoiners()
{
    uint32_t i;

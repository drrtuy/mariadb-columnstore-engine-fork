...
#ifndef CARTESIANJOIN_H_
#define CARTESIANJOIN_H_
...
class CartJoinStep : public JobStep, public TupleDeliveryStep
{
public:
    /**
     * @param
     */
    CartJoinStep(const JobInfo& jobInfo);
    virtual ~CartJoinStep();
...
private:
    CartJoinStep();
    CartJoinStep(const CartJoinStep&);
    CartJoinStep& operator=(const CartJoinStep&);
    ...
    struct HJRunner
    {
        HJRunner(CartJoinStep* hj) : HJ(hj) { }
        void operator()()
        {
            HJ->hjRunner();
        }
    ...
    struct SmallRunner
    {
        SmallRunner(CartJoinStep* hj, uint32_t i) : HJ(hj), index(i) { }
        void operator()()
        {
    ...
    struct JoinRunner
    {
        JoinRunner(CartJoinStep* hj, uint32_t i) : HJ(hj), index(i) { }
        void operator()()
        {
    ...
    struct DJSReader
    {
        DJSReader(CartJoinStep* hj, uint32_t i) : HJ(hj), index(i) { }
        void operator()()
        {
    ...
    struct DJSRelay
    {
        DJSRelay(CartJoinStep* hj) : HJ(hj) { }
        void operator()()
        {
            HJ->djsRelayFcn();
        }

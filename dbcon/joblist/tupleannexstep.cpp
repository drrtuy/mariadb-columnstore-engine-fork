...
void TupleAnnexStep::initialize(const RowGroup& rgIn, const JobInfo& jobInfo)
{
    ...
    if(fParallelOp && fOrderBy)
    {
        ...
        fSortingPlayGround.resize(fMaxThreads+1);
        for(id = 1; id <= fMaxThreads; id++)
        {
            ...
            fSortingPlayGround[id] = new std::vector<ordering::OrderByRow>();
            fSortingPlayGround[id]->reserve(18000000);
        }
    }
    else
    {
...
void TupleAnnexStep::executeParallelOrderBy(uint64_t id)
{
    ...
    std::vector<ordering::OrderByRow> *arrayPtr = fSortingPlayGround[id];
        ...
        while (more && !cancelled())
        {
            if (dlOffset%fMaxThreads == id-1)
            {
                ...
                for (uint64_t i = 0; i < rowCount; ++i)
                {
                    ...
                    arrayPtr->push_back(obRow);
                    r.nextRow(rowSize);
               }
            }
            sort(arrayPtr->begin(), arrayPtr->end());

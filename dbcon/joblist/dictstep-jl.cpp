...
messageqcpp::ByteStream DictStepJL::reencodedFilterString() const
{
    ...
    if (hasEqFilter)
    {
        ...
        for (uint32_t i = 0; i < filterCount; i++)
        {
            ...
            bs << eqOp;
            bs << roundFlag;
            bs << encodeStringPrefix((unsigned char*)eqFilter[i].c_str(), eqFilter[i].size(), charsetNumber);
        }
    }
    else
    {
        messageqcpp::ByteStream filterStringCopy(filterString); // XXX I am not sure about real semantics of messagecpp::ByteStream. So - copy.
        // please erfer to pdictionary.cpp in this dicrectory, addFilter function for a proper encoding of string filters.
        for (uint32_t i = 0; i < filterCount; i++)
        {
            ...
            // "Working with prefixes of 8 bytes" means these conditions reduce to ... WHERE col > 'customer' AND col < 'customer'
            // and their AND relation is impossible to satisfy.
            // We do not pass this string to primproc and that means we can reencode operation codes here.
            switch (cop)
            {
                case COMPARE_LT:
                case COMPARE_NGE:

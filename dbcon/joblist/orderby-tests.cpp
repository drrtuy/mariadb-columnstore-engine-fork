...
private:
   void orderByTest_nRGs(uint64_t numRows, uint64_t maxThreads, bool parallelExecution)
    {
        Timer timer;
        ...
        *(jobInfo.umMemLimit) = MEMORY_LIMIT;
        ...
        { 
            ...
            for(uint64_t i = 0; i < rowsPerRG; i++) // Worst case scenario for PQ
            {
                ...
                r.setUintField<8>(base + i, 0);
                r.setUintField<8>(base + i, 1);
                r.nextRow(rowSize);
            }
            base += 1;
    ...
    void ORDERBY_TIME_TEST()
    {
        ...
        orderByTest_nRGs(numRows * 14000, maxThreads, woParallel);
    }
    void QUICK_TEST()
    {
        float f = 1.1;

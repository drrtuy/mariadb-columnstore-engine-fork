...
Item* directRefItemOrItem(Item* item)
{
  if (((reinterpret_cast<Item_ref*>(item))->ref_type()) == Item_ref::DIRECT_REF)
    return item->real_item();
  return item;
...
Item* directRefItemOrNullLoop(Item* item)
{
  Item* refItem = item;
  while (item->type() == Item::REF_ITEM)
  {
    item = (((reinterpret_cast<Item_ref*>(item))->ref_type() == Item_ref::DIRECT_REF))
               ? item->real_item()
               : *((reinterpret_cast<Item_ref*>(item))->ref);
  }
  return refItem;
}
...
void check_sum_func_item(const Item* item, void* arg)
{
  ...
  {
    Item* ref_item_item = directRefItemOrItem(const_cast<Item*>(item));
...
void gp_walk(Item* item, void* arg)
{
                ...
                gp_walk_info* gwi)
{
  ...
  switch (itype)
  {
    ...
    case Item::REF_ITEM:
    {
      while (true)
      {
        Item_ref* ref = reinterpret_cast<Item_ref*>((directRefItemOrItem(item)));

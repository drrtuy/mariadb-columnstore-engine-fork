...
int ha_mcs_impl_create_(const char* name, TABLE* table_arg, HA_CREATE_INFO* create_info, cal_connection_info& ci)
{
    ...
    string db, tbl;
    if (table_arg->s->db.length && table_arg->s->table_name.length)
    {
    ...
    else
    {
        ...
        decode_file_path(name, decoded_dbname, decoded_tbname);
        db.assign(decoded_dbname); tbl.assign(decoded_tbname);
    }

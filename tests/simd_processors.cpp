...
TEST(SimdProcessorTest, Int8)
{
  ...
  // uint64_t* expectPtr = reinterpret_cast<uint64_t*>(&expectGt);
  // for (size_t i = 0; i < sizeof(IntegralType) >> 1; i++)
  //   if (l[i] > r[i])
  //   {
  //     std::cout << "1 l > r " << l[i] << " " << r[i] << std::endl;
  //     expectPtr[1] |= 1 << i;
  //   }
  // for (size_t i = sizeof(IntegralType) / 2; i < sizeof(IntegralType); i++)
  //   if (l[i] > r[i])
  //   {
  //     std::cout << "2 l > r " << l[i] << " " << r[i] << std::endl;
  //     expectPtr[0] |= 1 << i;
  //   }
  // auto res = proc.cmpGt(lhs, rhs);
  // uint64_t* resPtr = (uint64_t*)(&res);
  // std::cout << " expectPtr " << expectPtr[0] << " " << expectPtr[1] << std::endl;
  // std::cout << " res " << resPtr[0] << " " << resPtr[1] << std::endl;
  ...
  // auto v = _mm_cmpeq_epi8(lhs, rhs);
  // uint64_t* p = (uint64_t*)&v;
  // std::cout << std::hex << p[0] << " " << p[1] << std::endl;
  // uint64_t* p2 = (uint64_t*)&expectEq;
  // std::cout << std::hex << p2[0] << " " << p2[1] << std::endl;
  // auto res = proc.cmpEq(lhs, rhs);
  // uint64_t* resPtr = (uint64_t*)(&res);
  // uint64_t* expectPtr = reinterpret_cast<uint64_t*>(&expectEq);

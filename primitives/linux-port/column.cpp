...
// WIP make this a template with specs for KIND_FLOAT, IS_NULL == false and COL_WIDTH==4 or 8
...
// WIP make this a template with specs for KIND_TEXT, IS_NULL == false and COL_WIDTH==4 or 8
    else if (KIND_TEXT == KIND  &&  !IS_NULL)
    {
        if (COP & COMPARE_LIKE) // LIKE and NOT LIKE
        {
            utils::ConstString subject{reinterpret_cast<const char*>(&val1), COL_WIDTH};
            utils::ConstString pattern{reinterpret_cast<const char*>(&val2), COL_WIDTH};
            return typeHolder.like(COP & COMPARE_NOT, subject.rtrimZero(),
                                                      pattern.rtrimZero());
        }
        ...
        if (!rf)
        {
            ...
            if ((typeHolder.getCharset().state & (MY_CS_BINSORT|MY_CS_NOPAD)) ==
                (MY_CS_BINSORT|MY_CS_NOPAD))
              return colCompare_(order_swap(val1), order_swap(val2), COP);
            utils::ConstString s1{reinterpret_cast<const char*>(&val1), COL_WIDTH};
            utils::ConstString s2{reinterpret_cast<const char*>(&val2), COL_WIDTH};
            return colCompareStr(typeHolder, COP, s1.rtrimZero(), s2.rtrimZero());
        }
        else
            return colStrCompare_(order_swap(val1), order_swap(val2), COP, rf);
...
// WIP make this a template with specs for OTHER KINDS, IS_NULL == false || true and any COL_WIDTH
// partial spec
    else
    {
...
{
// WIP change this after the namespace change
    throw logging::NotImplementedExcept("getEmptyValue must not be used");
    return 42;
}

template<>
int64_t getEmptyValue<int64_t, 8>(uint8_t type)
{
    switch (type)
    {
        case CalpontSystemCatalog::DOUBLE:
...
template<>
int32_t getEmptyValue<int32_t, 4>(uint8_t type)
{
    switch (type)
    {
        case CalpontSystemCatalog::FLOAT:
...
template<>
int16_t getEmptyValue<int16_t, 2>(uint8_t type)
{
    switch (type)
    {
        case CalpontSystemCatalog::CHAR:
...
template<>
int8_t getEmptyValue<int8_t, 1>(uint8_t type)
{
    switch (type)
    {
        case CalpontSystemCatalog::CHAR:
...
// WIP Bit pattern representing EMPTY value for given column type/width
// Use typeHandler
template<>
int128_t getEmptyValue<int128_t, 16>(uint8_t type)
{
    return datatypes::Decimal128Empty;
}

template<int COL_WIDTH>
int128_t getNullValue(uint8_t type)
{
...
{
// WIP change this after the namespace change
    throw logging::NotImplementedExcept("getNullValue must not be used");
    return 42;
}

template<>
int64_t getNullValue<int64_t, 8>(uint8_t type)
{
    switch (type)
    {
        case CalpontSystemCatalog::DOUBLE:
...
template<>
int32_t getNullValue<int32_t, 4>(uint8_t type)
{
    switch (type)
    {
        case CalpontSystemCatalog::FLOAT:
...
template<>
int16_t getNullValue<int16_t, 2>(uint8_t type)
{
    switch (type)
    {
        case CalpontSystemCatalog::CHAR:
...
template<>
int8_t getNullValue<int8_t, 1>(uint8_t type)
{
    switch (type)
    {
        case CalpontSystemCatalog::CHAR:
...
inline bool isNullValue<KIND_TEXT, int64_t>(const int64_t val, const int64_t NULL_VALUE)
{
    ...
    int64_t ALT_NULL_VALUE = 0xFFFFFFFFFFFFFFFELL;

    // WIP check the second expr in the return cond
    return (val == NULL_VALUE ||
            val == ALT_NULL_VALUE);
}
...
inline bool matchingColValue(
    // Value description
    const T curValue,               // The value (IS_NULL - is the value null?)
    // Filter description
    const ColumnFilterMode columnFilterMode,
    const prestored_set_t* filterSet,     // Set of values for simple filters (any of values / none of them)
    ...
    const int64_t* filterValues,          //   value to compare to
    ...
    */
    switch (columnFilterMode)
    {
        ...
        case ONE_OF_VALUES_IN_ARRAY:
        {
            for (uint32_t argIndex = 0; argIndex < filterCount; argIndex++)
            {
                // WIP might produce an error for 16 bytes
    ...
    boost::shared_ptr<ParsedColumnFilter> parsedColumnFilter)
{
    ...
    auto filterValues  = (filterCount==0 ? nullptr : parsedColumnFilter->prestored_argVals.get());
    ...
    auto filterSet     = (filterCount==0 ? nullptr : parsedColumnFilter->prestored_set.get());

// ###########################
    // Bit patterns in srcArray[i] representing EMPTY and NULL values
    T EMPTY_VALUE = static_cast<T>(getEmptyValue<T, COL_WIDTH>(dataType));
    T NULL_VALUE  = static_cast<T>(getNullValue<T, COL_WIDTH>(dataType));
    ...
    std::cout << "filterColumnData width " << COL_WIDTH << " " << isNullValueMatches << "\n"; 
    ...
    T Max = datatypes::numeric_limits<T>::min();

// WIP make this an inline template
...
*/
                                    ...
                                    outputType, EMPTY_VALUE); )
    {
        ...
        else
        {
            ...
            if (ValidMinMax)
            {
                // WIP Make this branch a template
                if ((KIND_TEXT == KIND))
                {
                    if (colCompare<KIND, COL_WIDTH>(Min, curValue, COMPARE_GT, false, in->colType))
                        Min = curValue;

                    if (colCompare<KIND, COL_WIDTH>(Max, curValue, COMPARE_LT, false, in->colType))
                        Max = curValue;
                }
                else
                {
...
{
//WIP Might be useless in the context
